...
CACHE_VERSION = "20260722-long-chat-opportunities2"
...
def replace_ad(source: str, ad_id: str, *, label: str, title: str, description: str,
    ...
    def repl(match: re.Match[str]) -> str:
        ...
        tail = match.group(2)
        extras = ""
        for key in ("minimumMessages", "maximumShowsPerSession"):
            value = re.search(rf"{key}:(\d+)", tail)
            if value:
                extras += f",{key}:{value.group(1)}"
        return (
...
def patch_controller(source: str) -> str:
    ...
    ad_copy = {
        "inline_chat_1": dict(label="Quick Task", title="Complete Today’s 60-Second Challenge", description="Open this quick activity and collect 10 Activity Points for participating.", button="START QUICK TASK →", theme="emerald", icon="⚡"),
        "inline_chat_2": dict(label="Daily Discovery", title="A New Opportunity Just Unlocked", description="Take a look at today’s featured activity before it rotates out of your chat.", button="SEE WHAT UNLOCKED →", theme="violet", icon="✨"),
        "partner_list_1": dict(label="Community Pick", title="Popular With Active Members Today", description="Explore the opportunity other ChatEarn members are opening right now.", button="EXPLORE COMMUNITY PICK →", theme="blue", icon="🔥"),
        "inpage_1": dict(label="Point Mission", title="Complete This Optional Activity", description="Open the task, check what is available, and receive 10 Activity Points.", button="COMPLETE THE MISSION →", theme="gold", icon="🎯"),
        "inpage_2": dict(label="Surprise Unlock", title="Your Next Discovery Is Ready", description="A fresh activity has been selected for your current chat session.", button="REVEAL THE SURPRISE →", theme="rose", icon="🎁"),
        "button_ad_1": dict(label="Fast Action", title="One Tap Away From Your Next Task", description="Open this short activity without losing your conversation progress.", button="OPEN THE TASK →", theme="orange", icon="🚀"),
        "button_ad_2": dict(label="Member Access", title="An Extra Opportunity Is Available", description="Available to members who completed their first withdrawal journey.", button="ACCESS OPPORTUNITY →", theme="cyan", icon="🔓"),
        "half_screen_1": dict(label="Featured Challenge", title="Ready for a Different Kind of Task?", description="Pause for a moment, explore this featured activity, then continue your chat.", button="VIEW FEATURED CHALLENGE →", theme="indigo", icon="🏆"),
        "popup_1": dict(label="Limited Discovery", title="This Opportunity May Rotate Soon", description="Check the current activity while it is still available in this session.", button="CHECK IT NOW →", theme="magenta", icon="⏳"),
        "earnings_ad_1": dict(label="Activity Centre", title="Build Your Activity Point Balance", description="Explore the current task and collect 10 Activity Points when you open it.", button="OPEN ACTIVITY →", theme="teal", icon="💎"),
    }
