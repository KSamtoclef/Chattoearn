...
function maybeShowAd(){
 ...
 const ads=eligibleAds(AD_MANAGER.inlineChat).filter(a=>a.id!==state.ad.lastAdId);if(!ads.length){state.ad.replyCounter=0;state.ad.nextInterval=randomInterval();saveState();return}
...
function maybeHalfScreen(){
 const ads=eligibleAds(AD_MANAGER.halfScreen);if(!ads.length||sessionStorage.getItem('ce-half-screen-shown'))return;
...
function mountPartnerAd(){
 const ad=eligibleAds(AD_MANAGER.partnerList)[0],host=document.querySelector('#dashboard .foreigner-list');if(!ad||!host||$('partnerListAd'))return;
 const wrapper=adCard(ad,'partnerList');wrapper.id='partnerListAd';host.prepend(wrapper);
}
function mountEarningsAd(){
 const ad=eligibleAds(AD_MANAGER.earnings)[0],host=document.querySelector('#earnings');if(!ad||!host||$('earningsAd'))return;
