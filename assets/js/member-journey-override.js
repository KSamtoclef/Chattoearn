...
let conversationsCleaned=false;
...
function writeState(entry){if(entry?.key)localStorage.setItem(entry.key,JSON.stringify(entry.state))}
function activeScreen(){return document.querySelector('.screen.active')?.id||''}
function closeById(id){$(id)?.remove()}
function kycCompleted(s){
  const status=String(s?.kyc?.status||'').toLowerCase();
  return Boolean(
    s?.kyc?.returnedAt||
    ['returned_from_kyc','completed','approved','verified'].includes(status)||
    ['pending_review','approved'].includes(String(s?.paymentStatus||''))
  );
}
function removeVisibleAds(){
  document.querySelectorAll('[data-member-sponsored],#partnerListAd,#earningsAd,[data-ad-overlay],[data-ad-id],[data-temporary-ad]').forEach(node=>node.remove());
}
function removeStoredAds(s){
  let changed=false;
  if(!s?.conversations||typeof s.conversations!=='object')return changed;
  Object.keys(s.conversations).forEach(name=>{
    const messages=Array.isArray(s.conversations[name])?s.conversations[name]:[];
    const clean=messages.filter(message=>message?.type!=='ad');
    if(clean.length!==messages.length){s.conversations[name]=clean;changed=true}
  });
  return changed;
}
function syncMemberAds(resetSessionLocks=false){
  const entry=stateEntry();if(!entry)return false;
  const s=entry.state||{},allowed=kycCompleted(s);
  let changed=false;
  if(s.adsUnlocked!==allowed){s.adsUnlocked=allowed;changed=true}
...
function handleMemberReturn(){
  ...
  if(document.visibilityState==='visible'&&pageWasHidden){pageWasHidden=false;syncMemberAds(true)}
}
document.addEventListener('visibilitychange',handleMemberReturn);
window.addEventListener('pageshow',event=>{syncMemberAds(Boolean(event.persisted)||pageWasHidden);pageWasHidden=false});

function cleanDuplicateOpenings(){
  if(conversationsCleaned)return;
  const entry=stateEntry();if(!entry)return;
  const s=entry.state||{};let changed=false;
  Object.keys(s.conversations||{}).forEach(name=>{
    const messages=Array.isArray(s.conversations[name])?s.conversations[name]:[];
    const seenIds=new Set();let previousPartnerText='';
    const clean=messages.filter(message=>{
      if(message?.id&&seenIds.has(message.id)){changed=true;return false}
      if(message?.id)seenIds.add(message.id);
      const text=String(message?.text||'').trim();
      if(message?.type==='partner'&&text&&text===previousPartnerText){changed=true;return false}
      previousPartnerText=message?.type==='partner'?text:'';
      return true;
    });
    if(clean.length!==messages.length)s.conversations[name]=clean;
  });
  if(changed)writeState(entry);
  conversationsCleaned=true;
}

function showFirstWithdrawalGate(){
  if(gateOpen||$('firstWithdrawalGate'))return;
  gateOpen=true;
  const modal=document.createElement('div');modal.id='firstWithdrawalGate';
...
function enforceFirstWithdrawal(){
  const entry=stateEntry();if(!entry)return;
  const s=entry.state||{},available=Number(s.availableBalance??s.totalBalance??0);
  ...
  }else if(input){input.disabled=false;input.placeholder='Type a message...'}
}
...
function mountMemberHub(){
  const dashboard=$('dashboard'),host=dashboard?.querySelector('.bonus-banner'),entry=stateEntry();if(!host||!entry)return;
  const s=entry.state||{};let hub=$('memberHubAdvanced');if(!hub){hub=document.createElement('div');hub.id='memberHubAdvanced';host.after(hub)}
  ...
  hub.querySelector('[data-action="withdrawal"]').onclick=()=>s.withdrawal?window.goScreen?.(s.paymentStatus==='pending_review'?'processing':Number(s.sharing?.count||0)<REQUIRED_SHARE_ACTIONS?'sharewall':'kyc'):window.goScreen?.(Number(s.availableBalance||0)>=FIRST_WITHDRAWAL_MINIMUM?'withdraw':'earnings');
  hub.querySelector('[data-action="profile"]').onclick=()=>showProfile(s);
}
function showProfile(s){
  closeById('profileHubModal');const modal=document.createElement('div');modal.id='profileHubModal';modal.style.cssText='position:fixed;inset:0;z-index:99999;background:rgba(0,0,0,.82);display:grid;place-items:center;padding:20px';
...
function shareRulesAccepted(){return Boolean(stateEntry()?.state?.shareRulesAccepted===true)}
function markShareRulesAccepted(){const entry=stateEntry();if(!entry)return;entry.state.shareRulesAccepted=true;writeState(entry)}
...
function mountJourneyAd(screenId){
  ...
  const card=document.createElement('div');card.dataset.memberSponsored='1';card.style.cssText='margin:16px;padding:14px;border-radius:14px;background:#171b18;border:1px solid #303832';
  ...
  card.querySelector('button').onclick=()=>window.open('https://omg10.com/4/11279843','_blank','noopener,noreferrer');screen.appendChild(card);
}
function upgradeProcessingReturn(){
  ...
  button.dataset.memberReturn='1';button.textContent='RETURN TO MEMBER HOME';button.onclick=()=>{syncMemberAds(false);window.goScreen?.('dashboard');setTimeout(mountMemberHub,0)};
}
function tick(){
  const screen=activeScreen();
  configureShareRules();
  cleanDuplicateOpenings();
...
document.addEventListener('DOMContentLoaded',()=>{syncMemberAds(false);cleanDuplicateOpenings();tick()},{once:true});
})();